...
export const myOrders = (userId) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.get(`/api/v1/orders/me/${userId}`);
        dispatch({ type: SUCCESS_MY_ORDERS, payload: data.orders });
    } catch (error) {
        dispatch({
            type: FAIL_MY_ORDERS,
...
export const createOrder = (order) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.post("/api/v1/create_order", order, config);

        dispatch({ type: SUCCESS_CREATE_ORDER, payload: data });
    } catch (error) {
        dispatch({
