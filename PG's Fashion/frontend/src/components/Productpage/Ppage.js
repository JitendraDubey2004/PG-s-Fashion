...
import './Ppage.css'
import { BsHandbag, BsHeart, BsTag, BsCameraVideo } from 'react-icons/bs'
    ...
    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
      navigator.mediaDevices.getUserMedia({ video: true })
        ...
        .catch(err => {
          ...
          alert.show("Could not access camera");
        });
    }
  };
          ...
          <div>
            ...
            <div className='grid grid-cols-12 px-6 gap-8 mt-8'>
              ...
              <div className='col-span-5'>
                ...
                <div className='border-b-[1px] border-slate-200  pb-6 pt-4'>
                  ...
                  <button className="font1 font-semibold text-base py-4 px-8 inline-flex items-center justify-center border-[1px] border-slate-300 mt-4 mr-6 rounded-md hover:border-[1px] hover:border-slate-900"onClick={addtowishlist}><BsHeart className='mr-4' /><span>WISHLIST</span></button>
                  <button className="font1 font-semibold text-base py-4 px-8 inline-flex items-center justify-center border-[1px] border-[#0db7af] text-[#0db7af] mt-4 rounded-md hover:bg-[#0db7af] hover:text-white transition-colors" onClick={openAR}><BsCameraVideo className='mr-4' /><span>VIRTUAL TRY-ON</span></button>
                ...
                <div className='border-b-[1px] border-slate-200  pb-6 pt-4'>
                  ...
                  <li className='list-disc mt-2'>Coupon code: <span className='font-semibold'>PG250</span></li>
            ...
            {showAR && (
              <div className='fixed inset-0 z-50 bg-black/80 flex items-center justify-center overflow-hidden'>
                <div className='relative w-[90%] max-w-2xl bg-white rounded-lg p-4 flex flex-col items-center'>
                  <div className='absolute top-4 right-4 text-3xl cursor-pointer text-gray-500 hover:text-red-500 z-10' onClick={closeAR}>
                    <VscChromeClose />
                  </div>
                  <h2 className='text-2xl font1 font-bold mb-4 flex items-center gap-2'>
                    <BsCameraVideo /> Virtual Try-On <span className='text-xs bg-[#0db7af] text-white px-2 py-1 rounded-full'>BETA</span>
                  </h2>
                  <div className='relative w-full aspect-video bg-gray-200 rounded-lg overflow-hidden flex items-center justify-center'>
                    ...
                    {product.images && product.images[0] && (
                      <div className='absolute inset-0 flex items-center justify-center opacity-70 mix-blend-multiply'>
                        <img src={product.images[0].url} alt='Product Overlay' className='max-h-[60%] object-contain pointer-events-none drop-shadow-2xl' />
                      </div>
                    )}
                    <div className='absolute bottom-4 left-0 right-0 text-center pointer-events-none'>
                       <p className='text-white bg-black/50 inline-block px-4 py-2 rounded-full font1 text-sm'>
                          Step back and position yourself to try on {product.title.split(' ')[0]}
                       </p>
                    </div>
                  </div>
                </div>
