...
import { applyCoupon, removeCoupon, clearErrors as clearCouponErrors } from '../../action/couponaction';
...
const Bag = () => {
    ...
    const { couponInfo, error: couponError } = useSelector((state) => state.coupon);
...
const handleApplyCoupon = () => {
    if (couponCode.trim()) {
        dispatch(applyCoupon(couponCode, sp));
    }
};
        ...
        {/* ***************************************************************************************************************** */}
                ...
                <div className="col-span-5 mx-2 border-l-[0px] border-slate-200 pl-4 text-[14px] 2xl:ml-4 xl:ml-4 lg:ml-4 2xl:border-l-[1px] xl:border-l-[1px] lg:border-l-[1px]">
                     <div className='mt-2 '>
                         ...
                         <div className='text-[#535766] mt-4 mb-4 border-[1px] border-slate-200 p-2'>
                             <h1 className='font1 font-bold text-sm mb-2'>Apply Coupon</h1>
                             {couponInfo ? (
                                 <div className='flex justify-between items-center bg-green-50 p-2 border border-green-200'>
                                     <span className='font-semibold text-green-700'>{couponInfo.coupon} applied</span>
                                     <button className='text-red-500 font-bold text-xs' onClick={handleRemoveCoupon}>REMOVE</button>
                                 </div>
                             ) : (
                                 <div className='flex'>
                                     <input 
                                         ...
                                         className='border p-1 text-sm flex-1 outline-none uppercase'
                                     />
                                     <button 
                                         className='bg-[#ff3f6c] text-white px-3 text-sm font-bold ml-2'
                         ...
                         {couponInfo && (
                             <div className='text-[#535766] relative mt-2 text-[#0db7af]'> Coupon Discount<span className='absolute right-0'>-&#8377;&nbsp;{couponInfo.discountAmount}</span></div>
                         )}
                         ...
                         <div className='py-2 border-t-[1px] border-slate-200 bg-white 2xl:relative xl:relative lg:relative'>
                            <div className='relative font1 text-base font-bold'>Total Amount<span className='absolute right-0'>&#8377;&nbsp;{Math.round(sp - (couponInfo?.discountAmount || 0))}</span></div>
                            
                            <button className='mt-4 bg-[#ff3f6c] text-center w-full py-3 font1 text-bold text-white' onClick={place} > PLACE ORDER </button>
                         </div>
                    </div>   
